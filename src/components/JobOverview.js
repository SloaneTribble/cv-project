    ...
    job.editMode === false ? (
      <ul key={job.id} className="job">
        ...
        <div className="job-buttons">
          <button className={job.id} onClick={props.editJob}>
            Edit
          </button>
          <button className={job.id} onClick={props.deleteJob}>
            Delete
          </button>
        </div>
      </ul>
    ) : (
      <form key={job.id} className={job.id} onSubmit={props.submitEdit}>
        ...
        <input type="date" id="startDate" defaultValue={job.startDate}></input>
